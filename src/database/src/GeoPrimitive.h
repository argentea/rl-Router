...
class BoxOnLayer : public utils::BoxT<std::int64_t> {
public:
    int layerIdx{0};

    //  constructors
    template<typename... Args>
    BoxOnLayer(int layerIndex = -1, Args... params) : layerIdx(layerIndex), utils::BoxT<std::int64_t>(params...) {}

    // inherit setters from utils::BoxT in batch
    template<typename... Args>
    void Set(int layerIndex = -1, Args... params) {
        layerIdx = layerIndex;
        utils::BoxT<std::int64_t>::Set(params...);
    }
...
// helper
//utils::BoxT<std::int64_t> getBoxFromRsynBounds(const Bounds &bounds);
//utils::BoxT<std::int64_t> getBoxFromRsynGeometries(const vector<Rsyn::PhysicalViaGeometry> &geos);
...
template<>
struct hash<database::GridPoint> {
    std::size_t operator()(const database::GridPoint &gp) const {
